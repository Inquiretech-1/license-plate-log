...
3. **Add or update** [`master-log.json`](master-log.json) in the **`license-plate-log/`** directory (same format as the app’s JSON export: `version`, `exportedAt`, `entries`).  
...
3. **Añade o actualiza** [`master-log.json`](master-log.json) en el directorio **`license-plate-log/`** (mismo formato que la exportación JSON de la app: `version`, `exportedAt`, `entries`).  
